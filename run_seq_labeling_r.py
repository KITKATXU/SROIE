...
datasum=np.load('datasum.npy')
np.set_printoptions (threshold=np.inf)
errors={}
pretxt=open('pred.txt','a',encoding='utf-8')
w1=open('.errors_1.csv','w',encoding='utf-8',newline="")
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    # np.save('save_preds.npy',preds)
    preds=datasum
    preds = np.argmax(preds, axis=2)
    # print("preds=",'\n',preds)
    # pretxt.write(str(preds))
    ...
    np.save('out_label_ids.npy', out_label_ids)
            ...
            with open(
                ...
                for line in f:
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        ...
                        fd=0
                        for one in errors:
                            if line.split()[0] in errors.keys():
                                errors[line.split()[0]]=errors[line.split()[0]]+1
                                fd=1
                                break
                        if fd<1:
